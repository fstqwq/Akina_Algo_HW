...
According to the conclusion we proved in Problem 1.
...
where \( O(m(k - m) + m^2) = O(k^2)\) for all \(m\) satisfying \(1 \leq m < k\).

However, the above derivation doesn't work if \(m = n\), we need to elaborate it.

If \(m = n\), then \(r = n \% m = n - \lfloor \frac{n}{m} \rfloor m \leq n - m\), implies that \(r\) have at most \(n - 1\) bits.

Hence

\[
	T(k) = O(m(k - m)) + T(k - 1) = O((k-1)^2) = O(k^2)
\]
...
\end{python}
	...
	\begin{proof}
		...
		\[
			e^k \geq \frac{k^k}{k!} \Rightarrow \frac{1}{k!} \leq (\frac{e}{k})^k
		\]
