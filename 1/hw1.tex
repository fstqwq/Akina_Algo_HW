...
\end{python}
	...
	We now use `measure\_alg' to find the real running time of the algorithm, to check if we have the correct answer. To compare, we use $O(2^{n}k \log{\frac{n}{k}})$ as well as the LHS $$
	\sum_{i = 0}^{n} \sum_{j = 0}^k {n - i \choose k - j - n + i} \log{i \choose j}
	$$ as a `more precised' result, for it is not scaled to show a neat big-$O$ notation.
	...
	\begin{table}[htbp]
		...
		\end{tabular}%
		\caption{Running time analysis of recursion algorithm}
		\label{tab:addlabel}%
	\end{table}%
\end{problem}
...
However, we have a more efficient  -- $\Theta (\log n)$ -- conclusion:  $ {n \choose k} \bmod 2 = [n \wedge k = k]$.
	\begin{proof}
		...
		\begin{align*}
			...
			&\equiv {(a_u....a_3)_2 \choose (b_v....b_3)_2}{a_2 \choose b_2}{a_1 \choose b_1} \equiv ...\\	
