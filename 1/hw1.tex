...
\begin{problem}{3} 
    ...
    \solution
\begin{python}
# Recursive Algorithm for the Binomial Coefficient
def Rec(n, k):
    if k < 0 or k > n:
        return 0
    if n == 0:
...
\end{python}
    ...
    Notice that \( n \) always reduce by \( 1 \), and \( k \) always reduce by \(1\) or remain its value at each recursion, hence sub-recursion Rec \( (p, q) \) will be recalculated \( {n - p \choose k - q} \) times.
